...
from eval_satnerf import (
    find_best_embbeding_for_val_image,
...
class NeRF_pl(pl.LightningModule):
    ...
    def define_models(self):
        ...
        if self.args.model == "sat-nerf":
            self.embedding_t = torch.nn.Embedding(
                self.args.t_embbeding_vocab, self.args.t_embbeding_tau
            )
    ...
    def configure_optimizers(self):
        ...
        self.optimizer = torch.optim.Adam(
            parameters, lr=self.args.lr, weight_decay=0
        )
    ...
    def training_step(self, batch, batch_nb):
        ...
        ts = (
            None if not self.use_ts else batch["color"]["ts"].squeeze()
        )  # (B, 1)

        results = self(rays, ts)
        if (
            "beta_coarse" in results
            and self.get_current_epoch(self.train_steps) < 2
        ):
        ...
        if self.depth:
            ...
            kp_weights = (
                ...
                else torch.flatten(batch["depth"]["depths"][:, 1])
            )
        ...
        self.log(
            "train_psnr", psnr_, on_step=True, on_epoch=True, prog_bar=True
        )
        return {"loss": loss}
    ...
    def validation_step(self, batch, batch_nb):
        ...
        depth = train_utils.visualize_depth(
            results[f"depth_{typ}"].view(H, W)
        )  # (3, H, W)
        ...
        self.logger.experiment.add_images(
            "{}_{}/GT_pred_depth".format(split, sample_idx),
            stack,
            self.global_step,
        )
        ...
        if (
            (batch_nb == 0 or batch_nb == 1)
            and self.args.data == "sat"
            and save
        ):
            # save some images to disk for a more detailed visualization
            out_dir = (
                self.val_im_dir
                if self.is_validation_image
                else self.train_im_dir
            )
            save_nerf_output_to_images(
                self.val_dataset[0], batch, results, out_dir, epoch
            )
        ...
        if self.args.data != "sat":
            ...
            if batch_nb != 0:
                ...
                try:
                    ...
                    gt_roi_path = os.path.join(
                        self.args.gt_dir, aoi_id + "_DSM.txt"
                    )
                    gt_dsm_path = os.path.join(
                        self.args.gt_dir, aoi_id + "_DSM.tif"
                    )
                    assert os.path.exists(
                        gt_roi_path
                    ), f"{gt_roi_path} not found"
                    assert os.path.exists(
                        gt_dsm_path
                    ), f"{gt_dsm_path} not found"
                    ...
                    out_path = os.path.join(
                        self.val_im_dir,
                        "dsm/tmp_pred_dsm_{}.tif".format(unique_identifier),
                    )
                    _ = self.val_dataset[0].get_dsm_from_nerf_prediction(
                        rays.cpu(), depth.cpu(), dsm_path=out_path
                    )
                    mae_ = compute_mae_and_save_dsm_diff(
                        out_path,
                        batch["src_id"][0],
                        self.args.gt_dir,
                        self.val_im_dir,
    ...
    def get_current_epoch(self, tstep):
        return train_utils.get_epoch_number_from_train_step(
            tstep, len(self.train_dataset[0]), self.args.batch_size
        )
