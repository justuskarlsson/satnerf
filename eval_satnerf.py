...
def load_nerf(run_id, logs_dir, ckpts_dir, epoch_number):
    ...
    checkpoint_path = os.path.join(
        ckpts_dir, "{}/epoch={}.ckpt".format(run_id, epoch_number)
    )
    print("Using", checkpoint_path)
    if not os.path.exists(checkpoint_path):
        raise FileNotFoundError(
            "Could not find checkpoint {}".format(checkpoint_path)
        )
    ...
    if args.model == "sat-nerf":
        embedding_t = torch.nn.Embedding(
            args.t_embbeding_vocab, args.t_embbeding_tau
        )
...
def save_nerf_output_to_images(dataset, sample, results, out_dir, epoch_number):
    ...
    if "h" in sample and "w" in sample:
        ...
        W = H = int(
            torch.sqrt(torch.tensor(rays.shape[0]).float())
    ...
    try:
        dsm = dataset.get_dsm_from_nerf_prediction(
            rays.cpu(), depth.cpu(), dsm_path=out_path
        )
    except:
        dsm = None
    # save rgb image
    ...
    if f"sun_{typ}" in results:
        s_v = torch.sum(
            results[f"weights_{typ}"].unsqueeze(-1) * results[f"sun_{typ}"], -2
        )
        out_path = "{}/sun/{}_epoch{}.tif".format(out_dir, src_id, epoch_number)
        train_utils.save_output_image(
            s_v.view(1, H, W).cpu(), out_path, src_path
        )
        rgb_albedo = torch.sum(
            results[f"weights_{typ}"].unsqueeze(-1) * results[f"albedo_{typ}"],
            -2,
        )
        out_path = "{}/albedo/{}_epoch{}.tif".format(
            out_dir, src_id, epoch_number
        )
        train_utils.save_output_image(
            rgb_albedo.cpu().view(H, W, 3).permute(2, 0, 1).cpu(),
            out_path,
            src_path,
        )
        if f"ambient_a_{typ}" in results:
            a_rgb = torch.sum(
                results[f"weights_{typ}"].unsqueeze(-1)
                * results[f"ambient_a_{typ}"],
                -2,
            )
            out_path = "{}/ambient_a/{}_epoch{}.tif".format(
                out_dir, src_id, epoch_number
            )
            train_utils.save_output_image(
                a_rgb.view(H, W, 3).permute(2, 0, 1).cpu(), out_path, src_path
            )
            b_rgb = torch.sum(
                results[f"weights_{typ}"].unsqueeze(-1)
                * results[f"ambient_b_{typ}"],
                -2,
            )
            out_path = "{}/ambient_b/{}_epoch{}.tif".format(
                out_dir, src_id, epoch_number
            )
            train_utils.save_output_image(
                b_rgb.view(H, W, 3).permute(2, 0, 1).cpu(), out_path, src_path
            )
        if f"beta_{typ}" in results:
            ...
            out_path = "{}/beta/{}_epoch{}.tif".format(
                out_dir, src_id, epoch_number
            )
            train_utils.save_output_image(
                beta.view(1, H, W).cpu(), out_path, src_path
            )
        if f"sky_{typ}" in results:
            sky_rgb = torch.sum(
                results[f"weights_{typ}"].unsqueeze(-1) * results[f"sky_{typ}"],
                -2,
            )
            out_path = "{}/sky/{}_epoch{}.tif".format(
                out_dir, src_id, epoch_number
            )
            train_utils.save_output_image(
                sky_rgb.cpu().view(H, W, 3).permute(2, 0, 1).cpu(),
                out_path,
                src_path,
            )


def find_best_embbeding_for_val_image(
    models, rays, conf, gt_rgbs, train_indices=None
):
...
def find_best_embeddings_for_val_dataset(
    val_dataset, models, conf, train_indices
):
    ...
    for i in np.arange(1, len(val_dataset)):
        ...
        if aoi_id in ["JAX_068", "JAX_004", "JAX_214"]:
            ...
            ts = find_best_embbeding_for_val_image(
                models, rays, conf, rgbs, train_indices=train_indices
            )
...
def predefined_val_ts(img_id):
    ...
    if aoi_id == "JAX_068":
        ...
        d = {"JAX_004_022_RGB": 0, "JAX_004_014_RGB": 0, "JAX_004_009_RGB": 5}
    elif aoi_id == "JAX_214":
        d = {
            "JAX_214_020_RGB": 0,
            "JAX_214_006_RGB": 8,
...
def eval_aoi(
    ...
    if split == "train":
        ...
        dataset.json_files = [
            os.path.join(args.root_dir, json_p) for json_p in json_files
        ]
    ...
    for i in samples_to_eval:
        ...
        save_nerf_output_to_images(
            dataset, sample, results, out_dir, epoch_number
        )
        ...
        psnr.append(psnr_)
        ssim_ = metrics.ssim(
            results[f"rgb_{typ}"].view(1, 3, H, W).cpu(),
            rgbs.view(1, 3, H, W).cpu(),
        )
        ssim.append(ssim_)

        # geometry metrics
        pred_dsm_path = "{}/dsm/{}_epoch{}.tif".format(
            out_dir, src_id, epoch_number
        )
        mae_ = sat_utils.compute_mae_and_save_dsm_diff(
            pred_dsm_path, src_id, args.gt_dir, out_dir, epoch_number
        )
        mae.append(mae_)
        print(
            "{}: pnsr {:.3f} / ssim {:.3f} / mae {:.3f}".format(
                src_id, psnr_, ssim_, mae_
            )
        )

        # clean files
        in_tmp_path = glob.glob(os.path.join(out_dir, "*rdsm_epoch*.tif"))[0]
        out_tmp_path = in_tmp_path.replace(
            out_dir, os.path.join(out_dir, "rdsm")
        )
        os.makedirs(os.path.dirname(out_tmp_path), exist_ok=True)
        shutil.copyfile(in_tmp_path, out_tmp_path)
        os.remove(in_tmp_path)
        in_tmp_path = glob.glob(os.path.join(out_dir, "*rdsm_diff_epoch*.tif"))[
            0
        ]
        out_tmp_path = in_tmp_path.replace(
            out_dir, os.path.join(out_dir, "rdsm_diff")
        )
        os.makedirs(os.path.dirname(out_tmp_path), exist_ok=True)
        shutil.copyfile(in_tmp_path, out_tmp_path)
        os.remove(in_tmp_path)
